...
def make_markdown_table(
    ...
    assert all(len(row) == len(headers) for row in content), "Invalid table format."
...
class UpdatedTopic(Topic):
    ...
    def build(cls, raw_content: Mapping[str, Any]) -> UpdatedTopic:
        change_set = [
            ...
            ["Partition Count", raw_content["numPartitions"],raw_content["numPartitions"]],
        ...
        if raw_content["updatedConfigEntries"]:
            change_set.extend([
                [str(entry["name"]), str(entry["current"]), str(entry["updated"])] 
...
class TopicctlOutput:
    ...
    def build(cls, raw_content: str) -> TopicctlOutput:
        ...
        return TopicctlOutput(
            ...
            topics=[
                *[NewTopic.build(content) for content in parsed["newTopics"] or []],
                *[UpdatedTopic.build(content) for content in parsed["updatedTopics"] or []],
            ]
        )
...
def main():
    ...
    for topic in topics.topics:
        title = f"{dry_run}Topicctl ran apply on topic {topic.name} in region {SENTRY_REGION}"
